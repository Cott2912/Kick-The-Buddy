...
function WriteLazyLoadListToStorage(e) {
    return "undefined" == typeof localforage ? Promise.resolve() : localforage.setItem("", e)
}

function ReadLazyLoadListFromStorage() {
    return "undefined" == typeof localforage ? Promise.resolve([]) : localforage.getItem("")
}
...
async function CreateCacheFromFileList(e, t, a) {
    const n = await Promise.all(t.map((e => networkThrottle.Add((() => fetchWithBypass(e, a))))));
    let o = !0;
    for (const e of n) e.ok || (o = !1, console.error("[SW] Error fetching '" + e.url + "' (" + e.status + " " + e.statusText + ")"));
    ...
    try {
        ...
        throw console.error("[SW] Error writing cache entries: ", t), caches.delete(e), t
    }
}
async function UpdateCheck(e) {
    try {
        const t = await fetchWithBypass("offline.json", !0);
        if (!t.ok) throw new Error("offline.json responded with " + t.status + " " + t.statusText);
...
async function GetCacheNameToUse(e, t) {
    ...
    return console.log("[SW] Updating to new version"), await Promise.all(e.slice(0, -1).map((e => caches.delete(e)))), a
}
async function HandleFetch(e, t) {
    ...
    if (s) return s;
    const r = await Promise.all([fetch(e.request), ReadLazyLoadListFromStorage()]),
        i = r[0],
        c = r[1];
