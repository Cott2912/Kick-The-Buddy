...
'use strict'; {
    ...
    window.DOMElementHandler = class DOMElementHandler extends self.DOMHandler {
        ...
        AddDOMElementMessageHandlers(arr) {
            for (const [handler,
                    func
                ] of arr) this.AddDOMElementMessageHandler(handler, func)
        }
        _OnCreate(e) {
            const elementId = e["elementId"];
        ...
        _OnUpdatePosition(e) {
            ...
            if (fontSize !==
                null) elem.style.fontSize = fontSize + "em";
            const wantHtmlIndex = e["htmlIndex"];
        ...
        _OnRemoveAttribute(e) {
            const elem =
                this.GetElementById(e["elementId"]);
...
'use strict'; {
    ...
    window.RuntimeInterface = class RuntimeInterface {
        constructor(opts) {
            ...
            if (this._exportType === "playable-ad-single-file" || this._exportType === "playable-ad-zip" || this._exportType === "instant-games") this._useWorker = false;
            ...
            if (this.IsAnyWebView2Wrapper()) self["chrome"]["webview"].addEventListener("message", e => this._OnWrapperMessage(e.data, e["additionalObjects"]));
        ...
        async _Init(opts) {
            ...
            if (typeof window["AndroidFullScreen"] ===
                "object") try {
        ...
        async _InitWorker(opts, port2) {
            ...
            } else this._worker = await this.CreateWorker(workerMainUrl, {
                type: "module",
                name: "Runtime"
            });
        ...
        async _InitDOM(opts, port2) {
            ...
            if (projectMainScriptPath) try {
                await AddScript(projectMainScriptPath);
                if (this._exportType === "preview" && !globalThis.C3_ProjectMainScriptOK) throw new Error("main script did not run to completion");
            } catch (err) {
            ...
            if (scriptsInEventsPath) try {
                await AddScript(scriptsInEventsPath);
                if (this._exportType === "preview" && !globalThis.C3.ScriptsInEvents) throw new Error("scripts in events did not run to completion");
            } catch (err) {
                this._RemoveLoadingMessage();
        ...
        _OnCleanUpHTMLLayers() {
            ...
            this._pendingRemoveElements.length =
                0
        }
        ...
        _DoUpdateHTMLElementsZOrder() {
            ...
            if (s < i) this._DoUpdateHTMLElementsZOrderOnHTMLLayer(curHtmlIndex,
        ...
        _OnEventFromRuntime(e) {
            ...
            if (responseId === null) return ret;
                ...
                console.error(`Rejection from '${component}' handler '${handler}':`,
                    err);
        ...
        _OnResultFromRuntime(data) {
            ...
            const pendingPromise =
                pendingResponsePromises.get(responseId);
        ...
        async _MaybeGetPlatformSpecificScriptURL(url) {
            if (this._exportType ===
                ...
                if (this._localFileStrings.hasOwnProperty(url)) return {
                    isStringSrc: true,
                    str: this._localFileStrings[url]
                };
                else if (this._localFileBlobs.hasOwnProperty(url)) return URL.createObjectURL(this._localFileBlobs[url]);
        ...
        CordovaFetchLocalFile(filename) {
            const path = window["cordova"]["file"]["applicationDirectory"] +
                "www/" + filename;
            return new Promise((resolve, reject) => {
        ...
        _CordovaMaybeStartNextArrayBufferRead() {
            ...
            this._CordovaDoFetchLocalFileAsAsArrayBuffer(job.filename,
                job.successCallback, job.errorCallback)
        }
        ...
        async _CordovaDoFetchLocalFileAsAsArrayBuffer(filename, successCallback,
            errorCallback) {
            try {
                const file = await this.CordovaFetchLocalFile(filename);
            ...
            if (this._localFileBlobs.hasOwnProperty(url)) return this._localFileBlobs[url];
        ...
        _RequestPermission(id) {
            const api = this._GetPermissionAPI();
            return new Promise((resolve, reject) => api["requestPermission"](this._MapPermissionID(api,
                id), status => resolve(!!status["hasPermission"]), reject))
        }
        ...
        _OnWrapperMessage(msg, additionalObjects) {
            if (msg === "entered-fullscreen") {
                ...
                const type = msg["type"];
                if (type === "directory-handles") this._directoryHandles = additionalObjects;
                else if (type === "wrapper-init-response") {
                    this._wrapperInitResolve(msg);
                    this._wrapperInitResolve =
                        null
                } else if (type === "extension-message") this.PostToRuntimeComponent("runtime", "wrapper-extension-message", msg);
        ...
        async _ConvertDataUriToBlobs(filename, data) {
            if (typeof data === "object") {
                ...
                this._localFileStrings[filename] =
                    data["str"]
            } else {
...
'use strict'; {
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        ...
        _OnPointerEvent(name, e) {
            ...
            if (e.pointerType ===
                "mouse") this._mousePointerLastButtons = e.buttons
        }
...
'use strict'; {
    ...
    const HANDLER_CLASS = class BrowserDOMHandler extends self.DOMHandler {
        ...
        _OnSetWarnOnClose(e) {
            ...
            if (enabled) window.addEventListener("beforeunload", this._beforeunload_handler);
            else window.removeEventListener("beforeunload", this._beforeunload_handler)
        }
    };
    self.RuntimeInterface.AddDOMHandlerClass(HANDLER_CLASS)
};
